...
-- =============================================
--EXEC [sap].[usp_farmapronto_facturas] @AS400 = 0, @fecha_inicio = N'20230126', @fecha_fin = N'20230126'
CREATE PROCEDURE [sap].[usp_farmapronto_facturas] 
...
begin
     ...
     SELECT [Remision]
	   ...
	   --Where folio = 4411970 AND NoClienteProveedor = 'E09217'
      ...
      select @consulta='select   
			...
			from openquery(AS400,''
				...
				FROM MA4620EF04.SRBISH
      			...
      			where (OHODAT ='+@fecha_inicio+' and OHOTME >=60000 and IHTYPP=1) or (OHODAT ='+@fecha_fin+' and OHOTME < 60000 and IHTYPP=1)
			 	--RTRIM(IHINVN) = 805004411970
				union all
      			SELECT IHINVN,IHCUNO,ADWHCD,OHSURF,IHTYPP,OHORDT
				...
				--RTRIM(IHINVN) = 805004411970
			'') as x
			inner join [#facturas] on cast(IHINVN as varchar(20))=factura
			left join (
