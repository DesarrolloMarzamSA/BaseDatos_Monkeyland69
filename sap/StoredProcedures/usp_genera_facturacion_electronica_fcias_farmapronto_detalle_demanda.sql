...
CREATE  PROCEDURE [sap].[usp_genera_facturacion_electronica_fcias_farmapronto_detalle_demanda]
...
declare @fechaDiaUno varchar(8)=CONVERT(VARCHAR(8), dateadd(day,-60,@fecha), 112)
...
SELECT	'P' +
		...
		RIGHT(REPLICATE('0', 5) + CONVERT(VARCHAR(13), CONVERT(BIGINT,(DESCUENTOS * 100))), 5) +
		...
		RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR(13), CONVERT(BIGINT, (IMPORTE_BRUTO*CANTIDAD) * 100)), 8) +
