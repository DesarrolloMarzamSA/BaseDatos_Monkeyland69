...
CREATE PROCEDURE [sap].[usp_farmapronto_facturas_demanda] 
...
begin
     ...
     --   where FechaEmision between '2021-05-01' and '2021-08-04' and (serie='FC21' or serie='REMISION')
   ...
   --   			select 26 as sucursal_traductor,''V'' as ibs_letra
		 ...
		 -- 1.- LLenar la tabla temporal  		
		 insert into @extraccionEmbarque
		 -- exec  [192.168.90.209].[MiddleWare].[IEmbarque].[ExtraccionEmbarque] @fecha_inicio,'0011002195',''	
		 select distinct
			c.VWERK						,
			c.[PARTNER]					,
			'FC21' AS XBLNR				,
