...
-- =============================================
CREATE PROCEDURE [fbena].[IngresoHandHell] 
@nombre varchar(50),
...
BEGIN
    ...
    begin try
	    ...
	    if  not EXISTS (  select nombre from fbena.PedidosTransmitidos b where b.nombre=@nombre)		
		begin
			insert into  fbena.PedidosTransmitidos (nombre,rutalocal,rutaserver,enviado,fechaenvio,compania,error)
			values (@nombre,@rutalocal,@rutaserver,@enviado,getdate(),@compania,@error)
		end
		else
		begin
		update fbena.PedidosTransmitidos set enviado=@enviado,fechaenvio=getdate() where nombre=@nombre
		end
	end try
	begin catch
