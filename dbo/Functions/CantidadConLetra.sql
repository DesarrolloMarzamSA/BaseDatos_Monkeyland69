...
BEGIN    
	...
	DECLARE @lnEntero INT, @lcRetorno VARCHAR(512), @lnTerna INT,	@lcMiles VARCHAR(512),                        
			@lcCadena VARCHAR(512),	@lnUnidades INT, @lnDecenas INT, @lnCentenas INT, @lnFraccion INT        
	...
	WHILE @lnEntero > 0  
	BEGIN /* WHILE */            -- Recorro terna por terna            
		SELECT @lcCadena = ''            
		...
		SELECT @lnEntero = CAST(@lnEntero/10 AS INT)            -- Analizo las unidades            
		...
		CASE /* UNIDADES */              
			...
			ELSE @lcCadena            END /* UNIDADES */            -- Analizo las decenas
...
/* TERNA */            -- Armo el retorno terna a terna            
