...
CREATE PROCEDURE [dbo].[usp_ofertas_rama_sanpablo]
AS 
...
INNER JOIN dboferta t2 ON t1.codigo = t2.codigo and t2.sucursal = 1
...
select
	...
	--RIGHT(REPLICATE(' ', 4) + CONVERT(VARCHAR, CONVERT(INT, ROUND(porcentaje * 100, 0, 2))), 2) +
	RIGHT(REPLICATE(' ', 5) + convert(varchar(5), convert(money, porcentaje * 100)), 5) +
	CASE clas_abc 
	WHEN 'A' THEN '3'
	WHEN 'B' THEN '2'
	WHEN 'C' THEN 'A'
