...
/*
EXECUTE usp_lab_janssen_compras_ingresadas
*/
...
SELECT @cuantas = (SELECT COUNT(lab_corto) FROM lab_fusiones WHERE id_lab = 'JANSSEN')
...
IF (SELECT COUNT(*) FROM sysobjects WHERE name = 'lab_janssen_compras_ingresadas') = 0
	CREATE TABLE dbo.lab_janssen_compras_ingresadas(
...
if(@contaras > 10)
begin
	TRUNCATE TABLE lab_janssen_compras_ingresadas

	insert into lab_janssen_compras_ingresadas
	select 
	'Marzam' ClaveDist,
...
--select * into monkeyland.dbo.lab_janssen_compras_ingresadas from lab_janssen_compras_ingresadas
GO
