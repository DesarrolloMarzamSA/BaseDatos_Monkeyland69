...
CREATE	--	CREATE	--	DROP
PROCEDURE [dbo].[usp_lab_sanofi2_pedidos_26082013] (@fecha VARCHAR(10)) 
AS


/*
EXECUTE usp_lab_sanofi2_pedidos '2013-08-13'
SELECT * FROM lab_sanofi2_pedidos
SELECT * FROM lab_sanofi2_control_pedidos
*/
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_sanofi2_pedidos') 
	TRUNCATE TABLE lab_sanofi2_pedidos
ELSE
		CREATE --	DROP
		TABLE lab_sanofi2_pedidos (
...
/*	CASE	WHEN det.dest_det = 'FEA' THEN det.cant_ped
	...
	INNER JOIN #productos_sanofi2 mpb WITH (NOLOCK) ON	'00' + mpb.codigo = det.codigos
--	INNER JOIN sucursales_azm Z on Z.SUCURSAL = enc.sucursal

	WHERE 
	enc.sucursal =  @sucursal												AND
	enc.fechaprog = CONVERT(SMALLDATETIME,@fecha,121)
...
--	enc.fechaprog = convert(datetime, convert(varchar(10),current_timestamp,121), 121) and	--	'2009-05-27'
	AND det.dest_det in ('AAA', 'FEA', 'FEP')


--ALTER TABLE #tmp_lab_sanofi2_pedidos 
--	ADD PRIMARY KEY (ClaveSucDist, ClaveCliente, FechaDoc, Numero, CodigoMat, CodigoEAN, CantSolic)

	INSERT INTO lab_sanofi2_pedidos 
		SELECT 
			ClaveDist,
			ClaveSucDist,
			FechaDoc,
			...
			CantFaltante,
			MotFaltante
		FROM #tmp_lab_sanofi2_pedidos	;

	DROP TABLE #tmp_lab_sanofi2_pedidos
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_sanofi2_control_pedidos') 
	TRUNCATE TABLE lab_sanofi2_control_pedidos
ELSE
	CREATE TABLE lab_sanofi2_control_pedidos	(
...
SET @total_registros		= (SELECT ISNULL(COUNT(*)												,0)	FROM lab_sanofi2_pedidos)
SET @sum_ean						= (SELECT ISNULL(SUM(CONVERT(BIGINT,CodigoEAN))	,0)	FROM lab_sanofi2_pedidos)-- GROUP BY ean
SET @sum_cantidad				= (SELECT ISNULL(SUM(CantSolic)									,0)	FROM lab_sanofi2_pedidos)-- GROUP BY valor
SET @sum_precio					= (SELECT ISNULL(SUM(PrecioUnit)								,0)	FROM lab_sanofi2_pedidos)-- GROUP BY precio
SET @sum_valor					= (SELECT ISNULL(SUM(Valor)											,0)	FROM lab_sanofi2_pedidos)-- GROUP BY valor
SET @sum_cant_faltante	= (SELECT ISNULL(SUM(CantFaltante)							,0)	FROM lab_sanofi2_pedidos)-- GROUP BY valor


INSERT INTO lab_sanofi2_control_pedidos
SELECT
