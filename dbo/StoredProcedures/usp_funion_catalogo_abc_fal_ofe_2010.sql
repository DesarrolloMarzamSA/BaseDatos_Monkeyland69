...
/*
EXECUTE usp_funion_catalogo_abc_fal_ofe_2010 5
*/
--
...
/*
		usp_funion_catalogo_abc_fal_ofe_2010 5
*/

CREATE --	CREATE	--	DROP
PROCEDURE [dbo].[usp_funion_catalogo_abc_fal_ofe_2010] (@sucursal int)
AS
/*
...
CREATE TABLE #catalogo_maestro_farmacias_union (
  tipoMov						VARCHAR(1)	  ,      --  1
...
--  INNER JOIN dboferta ofe on mpb.codigo = ofe.codigo
  WHERE 
		mpb.codigo NOT IN (SELECT clave_proveedor FROM #catalogo_maestro_farmacias_union)
...
--    'XX' familia,
  FROM maestro_productos_baan mpb
--  INNER JOIN cat_productos_union tor on mpb.cod_barras_tandem = tor.cod_barras_tandem
  WHERE mpb.codigo NOT IN (SELECT clave_proveedor FROM #catalogo_maestro_farmacias_union)
...
WHERE codigo NOT IN (SELECT clave_proveedor FROM #catalogo_maestro_farmacias_union)
...
BEGIN
	...
	DELETE FROM #catalogo_maestro_farmacias_union 
...
--  INNER JOIN dboferta ofe on mpb.codigo = ofe.codigo
  ...
  WHERE i.piezas > 0
    AND mpb.codigo NOT IN (SELECT clave_proveedor FROM #catalogo_maestro_farmacias_union)
    AND ISNUMERIC(mpb.cod_barras_tandem) = 1 
    AND (CONVERT(bigINT, mpb.codigo) < dbo.gobierno() OR mpb.codigo IN (@excepciones))
	--LEFT(mpb.status,1) <> 'B'
...
--	fecha							smalldatetime,
	...
	EXECUTE usp_constructor_ofertas 'FUNION', @sucursal
	
-----------------------------------------------------------------------------------------------
UPDATE #catalogo_maestro_farmacias_union SET 
	porc_oferta = porcentaje * 100
FROM #catalogo_maestro_farmacias_union cat
INNER JOIN #funion_ofertas ofe ON ofe.sucursal = @SUCURSAL AND cat.clave_proveedor = ofe.codigo 
WHERE tipoMov <> 'F'
...
INNER JOIN catalogo_productos_funion f ON u.clave_proveedor = f.codigo
--	LEFT OUTER JOIN cat_prod_f_union F ON u.cod_barras_tandem = f.cod_barras_tandem
--	LEFT OUTER JOIN castigos_f_union H ON H.cod_barras_tandem = f.cod_barras_tandem
