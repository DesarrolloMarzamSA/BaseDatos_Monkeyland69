...
--select @descuento = '18.00'
  ...
  SELECT 
		left(convert(varchar(13), convert(bigint, t1.cod_barras)) + '             ', 13) + '|' +
            ...
            case
            when t1.codigo='2244201' then '000000000000.00'
            when t1.codigo='2244202' then '000000000000.00'
            else
				case 
				...
				when t1.clas_fis = 'F' then '000000000000.00'
				when t1.clas_fis = 'FA' then '000000000000.00'
				when t1.clas_fis = 'O'  then '000000000000.00'
				when t1.clas_fis = 'OA' then '000000000000.00'
            ...
            case isnull(LPLOQT,0) when 0 then '000000' else '999999' end + '|' +
            '0' + '|' +
            '0' + '|' +
  ...
  inner join   openquery(AS400,'
				select psprdc, case when psprdc = ''2244201'' then 47.00 when psprdc = ''2244201'' then 49.00 else COALESCE(PSSALP,0) end as prec_farm,PSPRIL from  MA4620EF04.SR4PRS where PSPRIL=''05''
  ...
  left join   openquery(AS400,'  select LPSROM,LPPRDC,SUM(integer(LPLOQT)) LPLOQT
 ...
 left outer join dboferta t3 on t1.codigo = t3.codigo and t3.sucursal=@sucursal and (t3.bolsa = 'C1007' or (t3.bolsa = 'XXPAD' and t3.codigo = '2750402'))
  where (LPSROM=@bodega and isnull(LPLOQT,0)>@cantidad_minima) or (t1.codigo in ('3800001','3800002'))
