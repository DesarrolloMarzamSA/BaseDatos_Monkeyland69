CREATE PROCEDURE [dbo].[usp_genera_respuesta_farmacon]
	@hash_md5 varchar(50)
as
...
update p set cantidad_surtida=d.IDQTY,p.motivo_no_surtido='8',tamano_archivo_respuesta=224,hora_resp_tandem=getdate(),rftp=getdate()
--SELECT p.*,d.[IDQTY]
  FROM [monkeyland].[dbo].[pedidos_farmacon] p with (nolock)
  inner join [dbo].[detalle_respuesta_farmacon] d
  on substring(p.cuenta,3,7)=substring(d.[IDCUNO],2,5) and p.codigo=rtrim(d.[IDPRDC]) and p.orden=rtrim(d.[IHOREF])
  where convert(varchar,FechaPedido,112)>=convert(varchar,getdate(),112)
...
select	right(replicate('0', 10) + convert(varchar(10), t1.num_pedido), 10) +
		...
		case
			...
			--when t1.motivo_no_surtido = 7 then '01'
