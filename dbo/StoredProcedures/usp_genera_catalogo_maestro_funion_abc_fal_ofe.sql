CREATE PROCEDURE [dbo].[usp_genera_catalogo_maestro_funion_abc_fal_ofe] (@sucursal int)	as


/*
EXECUTE usp_genera_catalogo_maestro_funion_abc_fal_ofe 5
*/
...
IF (SELECT COUNT(*) FROM sys.sysobjects WHERE name = 'catalogo_maestro_farmacias_union') = 0
create --	drop	--	truncate
table catalogo_maestro_farmacias_union (
	sucursal					int	,
...
--	TRUNCATE TABLE catalogo_maestro_farmacias_union
DELETE FROM catalogo_maestro_farmacias_union WHERE sucursal = @sucursal_final
...
insert into catalogo_maestro_farmacias_union   
  SELECT 
		@sucursal_final,
		CASE 
    ...
    right(REPLICATE(' ', 13)+Ltrim(Rtrim(cod_barras_tandem)), 13) as cod_barras_tandem,
    ...
    case when clas_fis IN ('B','BA','H','HA') THEN isnull(metodo_oferta.ofer,0) else 0 end as porc_oferta,
    0 escala_oferta,
    0 porc_oferta_2,
    0 escala_oferta_2,
    CASE  WHEN clas_fis IN ('B','BA') THEN @descto
          WHEN clas_fis IN ('N','NA') THEN 0
          WHEN clas_fis IN ('F','FA') THEN 0
          WHEN clas_fis IN ('O','OA') THEN 0
    ...
    grupo_est,																																
		...
		,	null,null,
		0,0,
		--isnull(metodo_oferta.ofer,0)/100,
		case when clas_fis IN ('B','BA','H','HA') THEN isnull(metodo_oferta.ofer,0)/100 else 0 end as oferta,
		CASE  WHEN clas_fis IN ('B','BA') THEN isnull(@descto,0)
          --WHEN clas_fis IN ('N','NA') THEN 0
          --WHEN clas_fis IN ('F','FA') THEN 0
          --WHEN clas_fis IN ('O','OA') THEN 0
          WHEN clas_fis IN ('H','HA') THEN isnull(descto_prod,0)
          Else 0 END/100,
        case when clas_fis IN ('B','BA','H','HA') THEN isnull(descu.descuento_volumen,0) else 0 end,
        --isnull(descu.descuento_volumen,0),
        case when clas_fis IN ('B','BA','H','HA') THEN isnull(descu.descuento_netos,0) else 0 end,
        --isnull(descu.descuento_netos,0),
          0
		--ISNULL(
  ...
  left join (
			...
			FROM [capa_ibs].[dbo].[tbl_metodos_ofertas]
			--where sucursal=@sucursal_final and metodo in ('ZF571','ZG571','ZFHYB','ZGMG2')
			where sucursal=5 and metodo in ('ZF571','ZG571','ZFHYB','ZGMG2')
			GROUP BY  codigo,porcentaje*100 ) as metodo_oferta
  on metodo_oferta.codigo=mpb.codigo
    left join catalogo_descuentos_farmacias_union descu on
		cast(mpb.codigo as bigint)=descu.codigo_marzam
  WHERE (mpb.codigo NOT IN (
			SELECT clave_proveedor 
			FROM catalogo_maestro_farmacias_union 
...
update catalogo_maestro_farmacias_union set descuento_final=1-((1-descuento1)*(1-descuento2)*(1-descuento3)*(1-descuento4)*(1-descuento5))
where sucursal=@sucursal_final

update catalogo_maestro_farmacias_union set precio_final=pcio_farmacia-(pcio_farmacia*descuento_final)
where sucursal=@sucursal_final
...
--	fecha							smalldatetime,
	...
	--INSERT INTO #funion_ofertas
	--EXECUTE usp_constructor_metodos_ofertas 'FUNION', @sucursal_final
	
-----------------------------------------------------------------------------------------------

--UPDATE catalogo_maestro_farmacias_union SET 
--	porc_oferta = ofe.porcentaje * 100,
--	metodo = ofe.bolsa,
--	vigencia_inicial = ofe.vigencia_inicial,
--	vigencia_final = ofe.vigencia_final
--FROM catalogo_maestro_farmacias_union cat
--INNER JOIN #funion_ofertas ofe ON 
...
FROM catalogo_maestro_farmacias_union U
...
drop table #funion_ofertas


--drop table catalogo_maestro_farmacias_union
