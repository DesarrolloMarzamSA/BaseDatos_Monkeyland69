...
CREATE --	CREATE	--	DROP
PROCEDURE [dbo].[usp_lab_merckmx_compras_ingresadas]
WITH ENCRYPTION
AS	/*
...
EXECUTE usp_lab_merckmx_compras_ingresadas
...
SELECT @cuantas = (SELECT COUNT(lab_corto) FROM lab_fusiones WHERE id_lab = 'MERCKMX')
...
IF (SELECT COUNT(*) FROM sysobjects WHERE name = 'lab_merckmx_compras_ingresadas') = 0
	CREATE TABLE dbo.lab_merckmx_compras_ingresadas(
...
if(@contaras > 10)
begin
	TRUNCATE TABLE lab_merckmx_compras_ingresadas

	insert into lab_merckmx_compras_ingresadas
	select 
	'Marzam' ClaveDist,
...
--select * into monkeyland.dbo.lab_merckmx_compras_ingresadas from lab_merckmx_compras_ingresadas
GO
