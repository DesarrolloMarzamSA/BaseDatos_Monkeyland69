...
CREATE PROCEDURE [dbo].[usp_genera_catalogo_minne]
--	@sucursal INT
--NOMBRE SP: 	usp_genera_catalogo_minne.      
--CLIENTE: 	FARMACIAS minne 
--DESCRIPCION: 	OBTIENE INFORMACION DE maestro_productos_baan 
--        PARA CREAR EL CATALOGO MAESTRO DE ARTICULOS PARA minne.
...
SELECT 
  ...
  CASE mpb.grupo_est																				
    WHEN 'PC01A' THEN mpb.prec_pub  + (mpb.prec_pub * 0.5)	
...
INNER JOIN inventario_baan invent						ON	mpb.codigo = invent.codigo 
  AND invent.sucursal = 23  --AND invent.piezas > 0
--	INNER JOIN maestro_grupos_estadisticos_baan mgeb	ON	mpb.grupo_est = mgeb.grupo_est	--	pedido por samy
WHERE convert(bigint,mpb.codigo) < dbo.gobierno()
AND isnumeric(mpb.codigo) = 1
--	AND mgeb.grupo_est = 'PJ01H' 	--	pedido por samy
...
INSERT INTO #tempo_mae_minne (col1)
  SELECT 
    ...
    RIGHT(REPLICATE('0',10) + CONVERT(varchar,cast(prec_pub  as decimal(7,2)) ) ,10) + @sep +	--	6
    RIGHT(REPLICATE('0',10) + CONVERT(varchar,cast(prec_farm as decimal(7,2)) ) ,10) + @sep +	--	5
    tip_prod + @sep +																					--	7
    tip_iva + @sep +																					--	8
    LEFT(clas_fis+REPLICATE(' ',2),2) + @sep +								--	9
    RIGHT(REPLICATE('0',6) + convert(varchar,descto),6)	+ '    '			--	10
  FROM #catalogo_minne

SELECT * FROM #tempo_mae_minne where col1 is not null
