...
GO
CREATE --	CREATE	--	DROP
PROCEDURE [dbo].[usp_lab_msd_movimientos_inventarios] (@fecha VARCHAR(10))
WITH ENCRYPTION
AS 

/*
EXECUTE usp_lab_msd_movimientos_inventarios '2010-11-02'
SELECT * FROM lab_msd_movimientos_inventarios
SELECT * FROM lab_msd_control_movimientos_inventarios
*/
...
EXECUTE usp_lab_msd_compras_ingresadas;

IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_msd_movimientos_inventarios') 
	TRUNCATE TABLE lab_msd_movimientos_inventarios
ELSE
	CREATE TABLE lab_msd_movimientos_inventarios (
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
		WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_msd_control_movimientos_inventarios') 
	TRUNCATE TABLE lab_msd_control_movimientos_inventarios

ELSE
	CREATE TABLE lab_msd_control_movimientos_inventarios (
...
--	VENTAS
INSERT INTO lab_msd_movimientos_inventarios (
...
SELECT 
	...
	NumeroLote,
		...
		' ' Texto2, 
		' ' Texto3
FROM lab_msd_transfer_sucursal
...
/*
UPDATE #tmp_lab_msd_compras
SET ClaveSucDist = AZM.AZM
FROM #tmp_lab_msd_compras compras	--WHERE
...
SELECT 
	...
	Texto2, 
	Texto3
 FROM #tmp_lab_msd_compras
--  WHERE

DROP TABLE #tmp_lab_msd_compras

DELETE FROM lab_msd_movimientos_inventarios WHERE rtrim(ltrim(ClaveDist)) = ''

UPDATE lab_msd_movimientos_inventarios SET Texto1 = '', Texto2 = '', Texto3 = ''
/*
UPDATE lab_msd_movimientos_inventarios SET ClaveSucDist = '17' WHERE ClaveSucDist = '50'
UPDATE lab_msd_movimientos_inventarios SET ClaveSucDist = '18' WHERE ClaveSucDist = '51'
UPDATE lab_msd_movimientos_inventarios SET ClaveSucDist = '19' WHERE ClaveSucDist = '52'
*/

-------	20	-	oct	-	2009
UPDATE lab_msd_movimientos_inventarios 
SET ClaveSucDist = RIGHT(REPLICATE('0',2) + CONVERT(VARCHAR,suc.almacen) ,2)
FROM lab_msd_movimientos_inventarios mov_inv
...
DECLARE cursor_sucursales CURSOR forward_only FOR 
	...
	FROM lab_msd_movimientos_inventarios
...
BEGIN
	SELECT * INTO #tmp_lab_msd_mi FROM lab_msd_movimientos_inventarios
		where ClaveSucDist = @sucursal and FechaMovimiento = @FechaMovimiento

	SET @total_registros = (SELECT COUNT(*) FROM #tmp_lab_msd_mi)
	SET @sum_ean = (SELECT Sum(convert(bigint,CodigoEAN)) FROM #tmp_lab_msd_mi)
	SET @sum_cantidad = (SELECT Sum(Cantidad) FROM #tmp_lab_msd_mi)

	INSERT INTO lab_msd_control_movimientos_inventarios
	SELECT
...
SET @estor = 'usp_lab_msd_movimientos_inventarios '+@fecha
