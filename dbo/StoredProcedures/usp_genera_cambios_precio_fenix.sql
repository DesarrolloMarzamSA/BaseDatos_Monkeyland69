...
--exec usp_genera_cambios_precio_fenix 17, '11560'
...
CREATE procedure [dbo].[usp_genera_cambios_precio_fenix] @sucursal int, @cliente varchar(5)
as
begin
...
right('000000000000' + convert(varchar(12), convert(money, case t2.grupo_est when 'PC01A' then t1.precio_publico_nuevo + (t1.precio_publico_nuevo * 0.5) else t1.precio_publico_nuevo end)), 9) +
right('000000000000' + convert(varchar(12), convert(money, case t2.grupo_est when 'PC01A' then t1.precio_farmacia_nuevo + (t1.precio_farmacia_nuevo * 0.5) else t1.precio_farmacia_nuevo end)), 9) 
from
