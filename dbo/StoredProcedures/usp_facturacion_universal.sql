...
-- =============================================
CREATE PROCEDURE [dbo].[usp_facturacion_universal] @bnd int
AS
BEGIN
	-- exec [usp_facturacion_universal] 1
	SET NOCOUNT ON;
...
where sucursal in(1,21) and cliente in('39910')  and convert(varchar,fecha_factura,112) between convert(varchar,getdate()-1,112) and convert(varchar,getdate(),112)
...
inner join rutas_facturacion_universal r on f.cliente=r.cliente and f.sucursal=r.sucursal
order by f.sucursal
end
