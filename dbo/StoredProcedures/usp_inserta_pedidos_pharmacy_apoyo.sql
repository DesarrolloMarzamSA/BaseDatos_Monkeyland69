...
CREATE procedure [dbo].[usp_inserta_pedidos_pharmacy_apoyo]
	@sucursal tinyint,
	@cliente varchar(15),
...
as	
	...
	--select * from capa_ibs..clientes_baan where ctepadre in ('959','917')  and cliente='86535'
	select	@x_Sucursal = sucursal	
	from capa_ibs..clientes_baan where ctepadre in ('959','917','258') and  cliente= @cliente
	
	select	@x_Cliente = cliente
	from capa_ibs..clientes_baan where ctepadre in ('959','917','258') and cliente=@cliente
	
	insert into	pedidos_pharmacy_apoyo
					(
						sucursal, 
						cliente,
						cod_barras, 
