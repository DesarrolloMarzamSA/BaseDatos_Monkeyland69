create PROCEDURE [dbo].[genera_reporte_cifrasdecontrol_piezasvendidas] @fecha_inicial char(8),@fecha_final char(8)
AS
...
select COUNT(IDSROM)
 ...
 from MA4620EF04.SROISDPL where IDIDAT BETWEEN '+@fecha_inicial+' and '+@fecha_final+'
...
select COUNT(IDSROM)
 ...
 from MA4620EF11.SROISDPL where IDIDAT BETWEEN '+@fecha_inicial+' and '+@fecha_final+'
...
SELECT almacen,codigo_cliente, codigo_producto, SUM(cantidad_pedida) AS cantidad_pedida,
   ...
   into #cifras_ventas
   FROM #cifras_temp
   WHERE fecha between @fecha_inicial and @fecha_final and tipo_documento1 = 'F' AND (tipo_documento2 = 1) AND (cuenta_padre not in (99995,99998,99940))
...
SELECT x.almacen,'000000' AS codigo_cliente, COUNT(x.almacen) AS codigo_producto,
 SUM(cantidad_pedida) AS cantidad_pedida, 0 AS cantidad_ofertada, SUM(cantidad_entregada) AS cantidad_entregada
  ...
  select distinct almacen,sum(cantidad_pedida) as cantidad from #reporte
  group by almacen
