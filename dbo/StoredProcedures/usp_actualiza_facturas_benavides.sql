CREATE PROCEDURE [dbo].[usp_actualiza_facturas_benavides]
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
	-- interfering with SELECT statements. select  CONVERT(varchar,getdate()-24,112)
	SET NOCOUNT ON;    
declare @sqlQuery varchar(max)
declare @fecha varchar(8)
set @fecha=   CONVERT(varchar,getdate()-3,112)
...
inner join MA4620EF04.SRONAM ON IDCUNO=NANUM
 ...
 where IHTYPP=1 and  NANCA1 in(''''99319'''',''''99199'''') and IHIDAT>='+@fecha+' 
 ...
 insert into #detalle_benavides  execute('select *  from openquery(as400, ''' + @sqlQuery + ''')')
  ...
  insert into monkeyland.[dbo].[detalle_benavides]
	...
	delete monkeyland..detalle_benavides where IDCUNO='G05961' and convert(varchar,FECHAPROG,112)=convert(varchar,getdate()-2,112)
	/*
...
CREATE TABLE #detalle_benavides2(
	...
	[IDCUNO] [char](11) NOT NULL ,
	[IDINVN] [numeric](20, 0) NOT NULL ,
	[FACTURA] [varchar](24) NOT NULL,
	[IDLINE] [numeric] NOT NULL,
	[IDPRDC] [char](35) NOT NULL ,
	[PCXPRC] [numeric](13, 0) NULL,
	...
	update d set d.sucursal=c.sucursal
		--select * 
		from #detalle_benavides2 d
		inner join [dbo].[cat_sucursales_benavides_sap] c on d.[IDCUNO]=c.cuenta
		 where len([IDCUNO])>6


		  ;with detalle_factura(Fila,SUCURSAL,SERIE,IDCUNO,IDINVN,FACTURA,IDLINE,IDPRDC,PCXPRC,IDDESC,IDQTY,CF,FARMACIA,UNITARIO,PRECIO_CANTIDAD,NETO_UNITARIO,NETO_CANTIDAD,IEPS,IEPS_MONEDA,IVA_MONEDA,FECHAPROG,DESCUENTOPROD,NOPEDIDO)
		 as (
		 select ROW_NUMBER() OVER(PARTITION BY  IDINVN,IDPRDC,IDLINE,IDQTY ORDER BY IDPRDC desc) AS Fila,SUCURSAL,SERIE,IDCUNO,IDINVN,FACTURA,IDLINE,IDPRDC,PCXPRC,IDDESC,IDQTY,CF,FARMACIA,UNITARIO,PRECIO_CANTIDAD,NETO_UNITARIO,NETO_CANTIDAD,IEPS,IEPS_MONEDA,IVA_MONEDA,FECHAPROG,DESCUENTOPROD,NOPEDIDO from #detalle_benavides2
		 )
