...
CREATE PROCEDURE [dbo].[usp_genera_catalogo_fahorro_cuauhtemoc]
--	@sucursal INT
--NOMBRE SP: 	usp_genera_catalogo_fahorrocuauhtemoc.      
--CLIENTE: 	FARMACIAS fcuauhtemoc 
--DESCRIPCION: 	OBTIENE INFORMACION DE maestro_productos_baan 
--        PARA CREAR EL CATALOGO MAESTRO DE ARTICULOS PARA F de Ahorro Cuauhtemoc.
...
INNER JOIN inventario_baan invent ON	mpb.codigo = invent.codigo 
  AND invent.sucursal = 24  AND invent.piezas > 0
...
INSERT INTO #tempo_mae_minne (col1)
  SELECT 
    ...
    RIGHT(REPLICATE('0',10) + CONVERT(varchar,cast(prec_farm as decimal(7,2)) ) ,10) + @sep +	--	6	--	A PETICION DEL CLIENTE
    tip_prod + @sep +																					--	7
    tip_iva + @sep +																					--	8
