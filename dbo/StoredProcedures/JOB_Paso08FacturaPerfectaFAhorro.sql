...
-- Description:	
/*
 EXEC [dbo].[JOB_Paso08FacturaPerfectaFAhorro]
*/
-- =============================================
CREATE PROCEDURE [dbo].[JOB_Paso08FacturaPerfectaFAhorro]
AS
BEGIN
	...
	@TotalMarzam decimal(20,2)=0.00
	 ...
	 WHILE EXISTS(SELECT TOP 1 1 FROM [dbo].[ControlFacturaPerfectaFAhorro] WHERE IdProceso = 8 AND IdEstado in (1,4) AND Procesar = 1)
	 BEGIN 
		SET @IdUsuario = NULL
		SET @Periodo  = NULL
        SET @Id = 0
		...
		SELECT TOP 1 @Id = Id, @IdUsuario = [Usuario], @Periodo = [Periodo] FROM [dbo].[ControlFacturaPerfectaFAhorro] WHERE IdProceso = 8 AND IdEstado in (1,4) AND Procesar = 1
		/*INICIA PROCESO */		
		BEGIN TRY
