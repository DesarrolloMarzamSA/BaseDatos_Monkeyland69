...
--[usp_genera_facturacion_electronica_minne]  '2021-06-29'

CREATE PROCEDURE [dbo].[usp_genera_facturacion_electronica_minne] ( @fecha varchar(10) )
AS
begin
...
set @fechaAux =  convert(varchar,getdate()-5,111)
--CREATE TABLE 
...
SELECT 
  ...
  convert(varchar,CONVERT(INT,f.importe_neto/  isnull(f.piezas_surtidas_con_cargo,1)       * @FACTOR )) precio_farm_sin_imp,
  ...
  --convert(varchar,CONVERT(INT,'0'              * @FACTOR )) descto_oferta,
  convert(varchar,CONVERT(INT,'0'            * @FACTOR )) descto_comercial,
  ...
  convert(varchar,CONVERT(INT,'0'   * @FACTOR )) porcentaje_descto_oferta,
  convert(varchar,CONVERT(INT,'0' * @FACTOR )) porcentaje_descto_comercial,
  f.cod_barras,
...
INTO #facturacion_minne --select f.*
FROM facturacion_electronica_estandar f
WHERE ctepadre = '717' AND segto in('C2' ,'A2') --order by fecha_factura desc
	AND fecha_factura >=CONVERT(datetime,@fechaAux,121)
