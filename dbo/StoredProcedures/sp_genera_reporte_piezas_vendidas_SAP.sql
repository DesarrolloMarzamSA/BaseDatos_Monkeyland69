...
--EXEC [dbo].[sp_genera_reporte_piezas_vendidas] '20200205'
...
CREATE PROCEDURE [dbo].[sp_genera_reporte_piezas_vendidas_SAP] 
...
cuentaMarzam varchar(500),
cuentaPadre varchar(500),
sucursal varchar(500)
)

insert into @cuentas
select rtrim(NANUM) as cuentaMarzam,
rtrim(NANCA1) as cuentaPadre,
rtrim(NOWHCD) as sucursal
from openquery(as400,'select NANUM,NANCA1,NOWHCD
from MA4620EF04.SRONAM
inner join MA4620EF04.SRONOI on NANUM=NONUM')

insert into #cifras_temp (almacen,codigo_cliente,codigo_producto,cantidad_pedida,cantidad_surtida,fecha,tipo_documento1,tipo_documento2,cuenta_padre,factura,linea_factura)

SELECT 
--[KUNAG]
c.sucursal as almacen
,b.[ALTKN] as codigoCliente
,RIGHT(MATNR,7) as codigoProducto
,cast(CANTIDAD as int) as pedida
,cast(CANTIDAD as int) as facturada
,convert(varchar(10),FKDAT,112) as fecha
,'F' as tipoDocumento
,1 as tipoDocumento2
,c.cuentaPadre as cuentaPadre
,VBELN
--,c.cuentaMarzam
,ROW_NUMBER() OVER(PARTITION BY VBELN    ORDER BY VBELN DESC) as lineaFActura
  FROM [192.168.90.209].[MiddleWare].[sapdo].[FacturasDevolver] as a
  inner join [192.168.90.209].[MiddleWare].[VwDM].[MClientes] as b on a.KUNAG=b.[PARTNER]
  inner join @cuentas as c on b.[ALTKN]=c.cuentaMarzam
  where FKDAT=CONVERT(Datetime,@fecha,112)

set @CountTemporal = (SELECT COUNT(1) FROM #cifras_temp)

--IF ( (SELECT SUM(registros) from @TableCountAs400) = @CountTemporal)
-- BEGIN
...
SELECT almacen,codigo_cliente, codigo_producto, SUM(cantidad_pedida) AS cantidad_pedida,
   ...
   WHERE fecha=@fecha and tipo_documento1 = 'F' AND (tipo_documento2 = 1) AND (cuenta_padre not in (99995,99998,99940))
    and almacen not in ('Y01','Y02','Y03','Y04','Y05','Y06','Y07','Y08','Y09',
					   'Z01','Z02','Z03','Z04','Z05','Z06','Z07','Z08','Z09','UWA','JWA','CWA','UFM','U01','U02','U03','U04','U05','U06','U07','U08','U09','U10','U11','U12','C01','C02','C03','M01') 
...
--   GROUP BY x.almacen
--   union
   select almacen+' |'+codigo_cliente+'    |'+'00000000000'+codigo_producto+'|'+right('               '+cast(cantidad_entregada as varchar(50)),15)+'.000' 
   from #cifras_ventas
