...
CREATE PROCEDURE [dbo].[usp_genera_catalogo_ofertas_farmacon_tijmed_000]
	@sucursal TINYINT,
	@primer_bolsa VARCHAR(5),
	@segunda_bolsa VARCHAR(5)
AS
--usp_genera_catalogo_ofertas_farmacon_tijmed_000 25, 'C2599', 'LIBRE'
...
SELECT 	RIGHT(REPLICATE('0', 7) + CONVERT(VARCHAR, t1.codigo), 7) +
		LEFT (CONVERT(VARCHAR, descripcion) + REPLICATE(' ', 31), 31) +
		CASE t1.grupo_est 
			WHEN 'PC01A' THEN RIGHT(REPLICATE('0', 7) + CONVERT(VARCHAR, CONVERT(MONEY, ROUND(t1.prec_farm, 2, 2) + (ROUND(t1.prec_farm, 2, 2) * 0.5))), 7)
			ELSE RIGHT(REPLICATE('0', 7) + CONVERT(VARCHAR, CONVERT(MONEY, ROUND(t1.prec_farm, 2, 2))), 7)
		END +
		'0000' +
		'0000' +
		CASE
