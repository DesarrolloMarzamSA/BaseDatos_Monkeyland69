...
CREATE	--	CREATE	--	DROP
PROCEDURE [dbo].[usp_chedraui_catalogo_maestro_2012]
AS

--	SELECT TOP 500 * FROM clientes_baan WHERE FARMACIA like '%ched%' ORDER BY fecha_alta

/*
EXECUTE usp_chedraui_catalogo_maestro_2012
*/

DECLARE @descto_cliente MONEY

SET @descto_cliente = (SELECT descuento FROM clientes_baan WHERE sucursal = 21 AND cliente = '99015')
...
SELECT
	...
	RIGHT(REPLICATE(' ', 06) +CONVERT(VARCHAR,ib.piezas)	,06)									AS exist_zona
