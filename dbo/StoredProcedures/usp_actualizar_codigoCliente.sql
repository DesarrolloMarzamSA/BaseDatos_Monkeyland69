...
-- =============================================
CREATE PROCEDURE [dbo].[usp_actualizar_codigoCliente]
AS
BEGIN
...
--delete 
  ...
  update pm set pm.sucursal=
			CASE
			WHEN RTRIM(LTRIM(NOI.noz3lent))='811' AND SUBSTRING(pm.clienteInterno,1,1)='D' THEN CAST('04' AS INT) 
			WHEN RTRIM(LTRIM(NOI.noz3lent))='807' AND SUBSTRING(pm.clienteInterno,1,1)='D' AND RTRIM(LTRIM(T2.CMCSTS))='075' THEN CAST('24' AS INT) 
			WHEN RTRIM(LTRIM(NOI.noz3lent))='817' AND SUBSTRING(pm.clienteInterno,1,1)='X' THEN CAST('23' AS INT) 
			WHEN RTRIM(LTRIM(NOI.noz3lent))='818' AND SUBSTRING(pm.clienteInterno,1,1)='X' THEN CAST('24' AS INT) 
			WHEN RTRIM(LTRIM(NOI.noz3lent))='875' THEN CAST('09' AS INT) 
