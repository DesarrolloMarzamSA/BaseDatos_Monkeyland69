...
*/
-- =============================================
CREATE PROCEDURE [dbo].[JOB_Paso10FacturaPerfectaFAhorro_Pruebas]
AS
BEGIN
    DECLARE
	 ...
	 BEGIN 
		...
		BEGIN TRY
		    UPDATE [dbo].[ControlFacturaPerfectaFAhorro] SET [Procesar] = 0 WHERE  Id = @Id
				...
				IF EXISTS(SELECT FTPER FROM OPENQUERY(AS400,'SELECT FTPER,FTSTS FROM MA4620EC.Z3OFACACUI') WHERE FTPER = @Periodo AND FTSTS = 0)
				BEGIN 
					PRINT ' => SI EXISTE u_U'
								UPDATE AS4
								SET 
				...
				ELSE
				BEGIN
						IF NOT EXISTS(SELECT FTPER FROM OPENQUERY(AS400,'SELECT FTPER FROM MA4620EC.Z3OFACACUI') WHERE FTPER = @Periodo)
						BEGIN
							PRINT ' => NO EXISTE :P'
							INSERT INTO 
             				OPENQUERY(AS400,'SELECT 
             									FTCSTS,FTPER,FTSER,FTINVN,FTFEC,FTRFC,FTTOB,FTTON,FTTIVA,FTTOI,FTTIV,FTBIV,FTIVA,FTDES,FTI08,FTINE,FTTIMP,FTBIE,FTI25,FTINI,FTTIMT,FTBAS,FTTIM,FTI00,FTTI0,FTBA0,FTFCH,FTHOR,FTSTS
             									FROM MA4620EC.Z3OFACACUI')
             				SELECT 
             					CASE  WHEN T.C13_Tasa_iva = 0.16 THEN '826'   
             							WHEN T.C13_Tasa_iva = 0.08 THEN '808' 
             					END FTCSTS,
