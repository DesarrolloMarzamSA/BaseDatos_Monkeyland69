...
-- =============================================
CREATE PROCEDURE [dbo].[usp_reporte_correo_apoyo] @parHashesMD5 varchar(max)
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
	-- interfering with SELECT statements.
	-- usp_reporte_correo_apoyo '''5ef411c301e9667f700f0b7a4d1bd454'',''5ef411c301e9667f700f0b7a4d1bd454'''
	...
	set @query='select  t1.arch_cliente ''Archivo Cliente'',isnull(t3.descripcion, ''Desconocido'') Sucursal,  
					    ...
					    from pedidos_pharmacy_apoyo t1 
				   ...
				   --print(@query)
				   execute (@query)
				   execute('update pedidos_pharmacy_apoyo set  enviado_ftp=2 where hash_md5 in ('+ @parHashesMD5 + ')')
				   execute('update pharmacy_lotes_apoyo set  finalizado=1 where hash_md5 in ('+ @parHashesMD5 + ')')
END
