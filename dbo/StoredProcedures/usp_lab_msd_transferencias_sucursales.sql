CREATE --	CREATE	--	DROP
PROCEDURE usp_lab_msd_transferencias_sucursales (@fecha VARCHAR(10))

AS 

/*
EXECUTE usp_lab_msd_transferencias_sucursales '2010-11-02'
SELECT * FROM lab_msd_transfer_sucursal
SELECT * FROM lab_msd_control_transfer_sucursal
*/
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_msd_transfer_sucursal') 
	TRUNCATE TABLE lab_msd_transfer_sucursal
ELSE
		CREATE TABLE lab_msd_transfer_sucursal (
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_msd_control_transfer_sucursal') 
	TRUNCATE TABLE lab_msd_control_transfer_sucursal

ELSE
	CREATE TABLE lab_msd_control_transfer_sucursal (
...
INSERT INTO lab_msd_transfer_sucursal
	SELECT --	TOP 25
	...
	INNER JOIN maestro_productos mpb ON '00' + mpb.codigo = det.codigos
...
--------------------------------------------------------------------------------
declare @cuenta int
set @cuenta = (select count(*) FROM lab_msd_transfer_sucursal)
if @cuenta = 0
	INSERT INTO lab_msd_transfer_sucursal
		SELECT 
			''		ClaveDist,	--		
...
SET @total_registros = (SELECT COUNT(*) FROM lab_msd_transfer_sucursal)
SET @sum_ean = (SELECT Sum(CONVERT(bigint,CodigoEAN)) FROM lab_msd_transfer_sucursal)
SET @sum_cantidad = (SELECT Sum(Cantidad) FROM lab_msd_transfer_sucursal)
...
---------------	20	OCT	2009	-------------------------
UPDATE lab_msd_transfer_sucursal SET 
	ClaveSucursalOrig = RIGHT(REPLICATE('0',2) + CONVERT(VARCHAR,suc.almacen) ,2)
FROM lab_msd_transfer_sucursal transfer
...
INSERT INTO lab_msd_control_transfer_sucursal
SELECT
	 @total_registros	TotalRegistros,
	 @sum_ean					SumaEAN,
	 @sum_cantidad		SumaCantidad

UPDATE lab_msd_transfer_sucursal SET NumeroLote = '', Texto1 = '', Texto2 = '', Texto3 = '' 

/*
UPDATE lab_msd_transfer_sucursal SET ClaveSucursalOrig = '17' WHERE ClaveSucursalOrig = '50'
UPDATE lab_msd_transfer_sucursal SET ClaveSucursalOrig = '18' WHERE ClaveSucursalOrig = '51'
UPDATE lab_msd_transfer_sucursal SET ClaveSucursalOrig = '19' WHERE ClaveSucursalOrig = '52'

UPDATE lab_msd_transfer_sucursal SET ClaveSucursalDest = '17' WHERE ClaveSucursalDest = '50'
UPDATE lab_msd_transfer_sucursal SET ClaveSucursalDest = '18' WHERE ClaveSucursalDest = '51'
UPDATE lab_msd_transfer_sucursal SET ClaveSucursalDest = '19' WHERE ClaveSucursalDest = '52'
*/

DECLARE @estor VARCHAR(50)
SET @estor = 'usp_lab_msd_transferencias_sucursales '+@fecha
