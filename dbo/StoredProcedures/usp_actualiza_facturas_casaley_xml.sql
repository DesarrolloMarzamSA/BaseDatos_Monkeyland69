...
CREATE PROCEDURE [dbo].[usp_actualiza_facturas_casaley_xml]
AS
BEGIN
...
insert into monkeyland.[dbo].[detalle_casaley_xml]
       ...
       left join  monkeyland.[dbo].[detalle_casaley_xml] d1 on d.IDINVN=d1.IDINVN
       where d1.IDINVN is null    
       drop table #detalle_benavides   
       delete monkeyland..detalle_casaley_xml where IDCUNO='G05961' and convert(varchar,FECHAPROG,112)>=convert(varchar,getdate()-2,112)
END
