CREATE PROCEDURE [dbo].[usp_genera_facturacion_electronica_funion] (@fecha varchar(10))	--	@sucursal int, 
AS
--------------------------------------------------------------------------------------------------
--	HECHO POR:	 MIGUEL SAMAYOA/ABRAHAM MARCELINO RAMRIEZ VEGA
/*
		EXECUTE usp_genera_facturacion_electronica_funion '2012-07-11'
*/
--  2009-12-16  SE CAMBIO EL FOLIO POR EL CFD A PETICION DE FUNION / PALMA.
--	2010-01-17	SE RELLENAN CON CEROS LOS CODIGOS DE BARRAS A PETICION DE FUNION / PALMA.
--	2011-01-24	SE RELLENAN CON CEROS LOS FOLIOS FISCALES A PETICION DE FUNION / ERICKA.
...
--	2012-07-11	SE RELLENAN CON CEROS LOS FOLIOS FISCALES A PETICION DE FUNION / PALMA.
--  2013-04-08  SE CAMBIA LOS CEROS A LA IZQUIERDA DEL CODIGO DE BARRAS YA QUE EL SISTEMA DE UNION LOS DETECTA COMO OTROS PRODUCTOS
--------------------------------------------------------------------------------------------------
/*
...
WHERE 
	...
	--e.segto = 'C2' AND 
	f.ctepadre IN ( '571', '012') and f.cliente not in (select cliente from cat_cuentas_fidealessureste)
...
--    RIGHT(REPLICATE('0',13) + LTRIM(RTRIM(f.cod_barras)),13)						AS codigo,								--	 3
    ...
    CASE WHEN F.clas_fis IN ('B','BA') THEN 18
         WHEN F.clas_fis IN ('N','NA','F','FA','O','OA') THEN 0
...
--  LEFT OUTER JOIN catalogo_productos_funion u ON u.codigo = f.codigo
