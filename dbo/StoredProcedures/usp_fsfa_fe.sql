...
--  FECHA CREACION: 27-FEB-2009

/*

usp_fsfa_fe 3, '2011-09-14'
*/

CREATE --	CREATE
PROCEDURE [dbo].[usp_fsfa_fe] (@sucursal INT , @FECHA VARCHAR(10))
AS
...
--	F.tipo_documento,
	F.folio_fiscal 
INTO #sfco_hoy2--select *
FROM facturacion_electronica_estandar f
WHERE f.sucursal = @sucursal
  AND convert(varchar,f.fecha_tandem,112) >= convert(varchar,getdate(),112)
  AND f.ctepadre = '231'
  --and f.factura in('00061841','00062251')
  ORDER by factura,orden

  select distinct * INTO #sfco_hoy from #sfco_hoy2

ALTER TABLE #sfco_hoy ADD PRIMARY KEY (sucursal, cliente, factura, codigo)
...
INSERT INTO #facturacion_electronica_san_francisco_asis (col1, col2, col3, col4)
(SELECT primer_iden, proveedor, fecha_disco, total_registros FROM #primer_renglon_sf)
...
BEGIN
  INSERT INTO #facturacion_electronica_san_francisco_asis
    SELECT  
      registro   ,																																				--	1
      RIGHT(REPLICATE('0',10) + right(factura,7)                    ,10)  ,								--	2
      factura_tienda  ,																																		--	3
      factura_sec_depto  ,																																--	4
      fecha_entrega  ,																																		--	5
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,importe_a_pagar    ),12) ,								--	6
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,importe_descuento  ),12) ,								--	7
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,importe_total      ),12) ,								--	8
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,importe_iva        ),12) ,								--	9
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,importe_ieps       ),12) ,								--	10
      RIGHT(REPLICATE('0', 6) + CONVERT(VARCHAR,total_productos    ), 6) ,								--	11
      ceros  ,																																						--	12
      cliente,																																						--	13
      NULL,																																								--	14
      null,																																								--	15
      ...
      null,																																								--	17
      @contador																																						--	18
--      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,descuento_tasa_10  ),11)  ,
    FROM #encab_sanfco
  ...
  INSERT INTO #facturacion_electronica_san_francisco_asis
    SELECT  
      indica                                                             ,								--	1
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,venta_tasa_0       ),11)  ,								--	2
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,descuento_tasa_0   ),11)  ,								--	3
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,venta_tasa_10      ),11)  ,								--	4
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,iva_tasa_10        ),11)  ,								--	5
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,descuento_venta_10 ),11)  ,								--	6
      RIGHT(REPLICATE('0',11) + CONVERT(VARCHAR,iva_descto_vta_10  ),11)  ,								--	7
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,iva_global         ),12)  ,								--	8
      RIGHT(REPLICATE('0',12) + CONVERT(VARCHAR,iva_descto_global  ),12)  ,								--	9
      RIGHT(REPLICATE('0', 4) + CONVERT(VARCHAR,numero_de_bolsas   ), 4)  ,								--	10
      RIGHT(REPLICATE('0', 5) + CONVERT(VARCHAR,numero_de_bultos   ), 5)  ,								--	11
      ceros,																																							--	12
      NULL,																																								--	13
      null,																																								--	14
      null,																																								--	15
  ...
  INSERT INTO #facturacion_electronica_san_francisco_asis
    SELECT  
      articulo  ,																																					--	1
      RIGHT(REPLICATE('0', 8) + cod_sanfco                               , 8)  ,   				--	2
      RIGHT(REPLICATE('0', 6) + CONVERT(VARCHAR,can_sanfco)              , 6)  ,   				--	3
      RIGHT(REPLICATE('0', 6) + CONVERT(VARCHAR,oferta)                  , 6)  ,   				--	4
      RIGHT(REPLICATE('0', 5) + CONVERT(VARCHAR,porcentaje_descto_oferta), 5)  ,   				--	5
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,precio_farm_sin_imp)     , 8)  ,   				--	6
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,precio_pub_con_imp)      , 8)  ,   				--	7
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,imp_farmacia_por_renglon), 8)  ,   				--	8
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,imp_publico_por_renglon) , 8)  ,   				--	9
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,imp_iva_por_renglon)     , 8)  ,   				--	10
      RIGHT(REPLICATE('0', 8) + CONVERT(VARCHAR,imp_ieps_por_renglon)    , 8)  ,   				--	11
      ceros                                                                    ,   				--	12
      RIGHT(REPLICATE('0',14) + aemecop                                  ,14)  ,   				--	13
      RIGHT(REPLICATE('0', 4) + CONVERT(VARCHAR,margen_utilitario)       , 4)  ,   				--	14
      clas_fis                                                                 ,					--	15
      referencia                                                               ,					--	16
      CONVERT(VARCHAR,venta_neta),																												--	17
      contador																																						--	18
...
FROM #facturacion_electronica_san_francisco_asis 
