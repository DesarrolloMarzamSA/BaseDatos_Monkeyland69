...
GO
CREATE	--	CREATE	--	DROP	
PROCEDURE [dbo].[usp_chedraui_pedidos_insert] 
...
SET @tienda								= SUBSTRING(@linea_buffer , 25, 05) 

SET @compania							= SUBSTRING(@linea_buffer , 01, 03) 
SET @proveedor						= SUBSTRING(@linea_buffer , 04, 06) 
SET @orden_pharmacy				= SUBSTRING(@linea_buffer , 10, 10) 
SET @bodega								= SUBSTRING(@linea_buffer , 20, 05) 
SET @upc									= SUBSTRING(@linea_buffer , 30, 13) 
SET @cantidad_solicitada	= CONVERT(INT , SUBSTRING(@linea_buffer , 43, 07) )
...
INSERT INTO pedidos_chedraui_v3	
(
	arch_cliente				,
	linea_buffer					,
	linea									,
