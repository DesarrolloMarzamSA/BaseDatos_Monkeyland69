CREATE procedure usp_genera_reporte_venta_fahorro_franquicias
	@fecha varchar(10),
	@segto varchar(12),
	@ctepadre varchar(3)
as
...
select	t1.cliente CLIENTE, 
		...
		t1.codigo CODIGO, 
		t1.cant_ped PIEZAS,
		t1.arch_cliente ORDEN,
		@fecha FECHA
into	#t1
from	pedidos_fahorro_franquicias t1 inner join Capa_ibs.dbo.pedidos_traductor t2 on
...
where	isnull(t2.motivo_no_surtido, '') = 'AAA' and
		convert(datetime, convert(varchar(10), t1.fecha_pedido, 121), 121) = convert(datetime, convert(varchar(10), dateadd(dd, -1, @fecha), 121), 121)
select	t1.cliente CLIENTE,
		t1.factura FACTURA,  
		t1.sucursal, 
...
select	@sql =	@sql + convert(varchar, dateadd(dd, -1,  convert(datetime, @fecha, 121)), 112) + 
				...
				' AND SROLSTLS.LSLSRN NOT IN (''''DIERR'''') AND SROLSTLS.LSCCA1 = ''''99008'''''
