...
/*
usp_lab_sanofi_invoice_creation 7, '2011-02-11'
*/
...
----WHERE 
----	f.fecha_tandem = CONVERT(DATETIME,@fecha,121)
...
--FROM #facturas_sanofi facturas
--INNER JOIN sucursales suc ON facturas.sucursal = suc.sucursal
...
INSERT INTO #facturas	(sucursal, cliente, factura)
	SELECT DISTINCT fe.sucursal, fe.cliente, fe.factura 
	FROM fes_sanofi	fe
	INNER JOIN sucursales s											WITH(NOLOCK)	ON s.almacen = @almacen AND s.sucursal = fe.sucursal
	INNER JOIN lab_sanofi_cat_productos mpb 		WITH(NOLOCK)	ON mpb.codigo = fe.codigo
...
BEGIN
	...
	WHERE f.fecha_factura = CONVERT(DATETIME,@fecha,121) AND
	f.sucursal = @sucursal AND f.cliente = @cliente AND f.factura = @factura
	...
	BEGIN
		INSERT INTO #domicilios
			SELECT
				...
				cb.direccion,
				cb.colonia,
				REPLACE(cb.poblacion,',','') poblacion  ,
				'',	--	cb.poblacion,		
				cb.codigo_postal ,
...
--	SELECT * FROM #facturas_sanofi order by sucursal, cliente, factura
