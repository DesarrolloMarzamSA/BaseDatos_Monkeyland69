...
-- =============================================
CREATE PROCEDURE [dbo].[usp_farmapronto_facturasCZ] --1,
...
begin
     ...
     SELECT [Remision]
      ...
      from openquery(AS400,''
      	   ...
      	   INNER JOIN MA4620EF04.SRONAM ON OHCUNO=NANUM AND NANCA1 IN (''''99676'''')
      	   ...
      	   INNER JOIN MA4620EF11.SRONAM ON OHCUNO=NANUM AND NANCA1 IN (''''99676'''')
      	   WHERE (OHODAT ='+@fecha_inicio+' and OHOTME >=60000 and IHTYPP=1) or 
				 (OHODAT ='+@fecha_fin+' and OHOTME < 60000 and IHTYPP=1)
				 '') as x
			 left join [#facturas] on IHINVN=factura
      	     left join (
