CREATE PROCEDURE [dbo].[usp_genera_cambios_precio_fenix_ventas] @sucursal int, @cliente VARCHAR(5)
AS
BEGIN

--  ESTE ES EL CATALOGO QUE SE LE ENVIA POR MAIL A VICKY
--	MODIFICADO	2010-03-25	FUNCION GOBIERNO



--exec usp_genera_cambios_precio_fenix_ventas 17, '11560'
--SELECT * FROM cambios_precio_baan WHERE t_item = '0525233'
--SELECT top 100 * FROM encabezado WHERE sucursal = 7 AND farmacia like '%fenix%'
...
CREATE TABLE #cambios_fenix(tipo VARCHAR(1), 
...
DECLARE cur_cambios CURSOR fast_forward FOR 
  SELECT t_item, t_prfn, t_prpn, max(fecha_hora) fecha_hora 
   ...
   WHERE fecha_hora > dateadd(dd, -10, current_timestamp) 
...
BEGIN
  ...
  GROUP BY t_prfn, t_prpn
	INSERT INTO #cambios_fenix(
		tipo, codigo, precio_farmacia_anterior, precio_farmacia_nuevo, precio_publico_anterior, precio_publico_nuevo, fecha) 
...
UPDATE #cambios_fenix SET 
  ...
  cod_barras = t2.cod_barras, nombre = t2.descripcion, iva = t2.iva, descuento =  
	...
	clas_fis = t2.clas_fis
FROM #cambios_fenix t1 
...
--  SELECT * FROM #cambios_fenix -- where precio_publico_anterior IS NULL


SELECT 
  RIGHT(REPLICATE('0', 7) + t1.codigo    ,  7) codigo    ,
  RIGHT(REPLICATE('0',13) + t1.cod_barras, 13) cod_barras,
  LEFT(t1.nombre + REPLICATE(' ',50), 30) descripcion,
  LEFT(t1.laboratorio + REPLICATE(' ',50), 30) laboratorio,
  ...
  RIGHT(REPLICATE(' ', 9) + CONVERT(VARCHAR, CONVERT(MONEY, 
    CASE t2.grupo_est WHEN 'PC01A' THEN t1.precio_publico_anterior + (t1.precio_publico_anterior * 0.5) 
    ELSE t1.precio_publico_anterior END)), 9) precio_publico_anterior,
  ...
  RIGHT(REPLICATE(' ', 9) + CONVERT(VARCHAR, CONVERT(MONEY, 
    CASE t2.grupo_est WHEN 'PC01A' THEN t1.precio_farmacia_anterior + (t1.precio_farmacia_anterior * 0.5) 
    ELSE t1.precio_farmacia_anterior END)), 9) precio_farmacia_anterior,
