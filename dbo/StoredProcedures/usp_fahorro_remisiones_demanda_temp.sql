...
set @sql='select * into ##encabezado2 from historica.dbo.encabezado with(nolock) where segto = ''C1'' and ctepadre = ''007'' and  horacap>=''2013-05-21 06:00:00'' and horacap<=''2013-05-22 05:59:59''  AND  ISNUMERIC(LTRIM(RTRIM(ORDEN))) = 1  '
set @sql2='select t2.* into ##detalle2 from historica.dbo.encabezado t1 with(nolock) inner join historica.dbo.detalle t2 with(nolock) on t1.sucursal = t2.sucursal and t1.serie = t2.serie and t1.factura = t2.factura where t1.horacap>=''2013-05-21 06:00:00'' and horacap<=''2013-05-22 05:59:59'' and t1.segto = ''C1'' and t1.ctepadre = ''007'' AND ISNUMERIC(LTRIM(RTRIM(T1.ORDEN))) = 1 '
--print(@sql)
end
else if(@tipo=2)
begin
...
t8.remisionado
from 
(select * from historica.dbo.encabezado with(nolock) where segto = 'C1' and ctepadre = '007' and fecha_tandem = convert(smalldatetime, '2013/02/27' , 121) AND  ISNUMERIC(LTRIM(RTRIM(ORDEN))) = 1) t1
--set @fechaAux=convert(smalldatetime,CONVERT(smalldatetime,@fecha,121)-2,121)
--set @fechaAux1=convert(smalldatetime,CONVERT(smalldatetime,@fecha,121)+1,121)
 inner join (select t2.* from historica.dbo.encabezado t1 with(nolock) inner join historica.dbo.detalle t2 with(nolock) on t1.sucursal = t2.sucursal and t1.serie = t2.serie and t1.factura = t2.factura where t1.segto = 'C1' and t1.ctepadre = '007' AND ISNUMERIC(LTRIM(RTRIM(T1.ORDEN))) = 1) t2 
 on t1.sucursal = t2.sucursal and t1.factura = t2.factura  
inner join capa_ibs.dbo.maestro_productos t3 with(nolock)  on t2.codigos = '00' + t3.codigo  
left outer join clientes_baan t6 with(nolock) on t1.sucursal = t6.sucursal and t1.cliente = t6.cliente  
inner join pedidos_spt_fahorro t8 with(nolock) on cast(t8.orden as int) = cast(t1.orden as int) and '00' + t8.codigo = t2.codigos and t1.cliente = t8.cuenta and t8.sucursal = t1.sucursal
left outer join devoluciones_spt_fahorro t9 with(nolock) on t9.devfa_sucursal = t2.sucursal and  t9.devfa_folio_remision = t2.factura and '00' + t9.devfa_codigo = t2.codigos and t9.devfa_tipo_reclamacion not in ('CR', 'NC') and t9.devfa_fecha_problema is null
inner join iva_sucursales t10 with(nolock) on t1.sucursal = t10.sucursal 
where 
(convert(int, t3.codigo) < dbo.gobierno() or t3.codigo = '9000701') and 
...
t2.dest_det = 'AAA' and 
t1.orden <> '' 
and cuenta_estilo_ahorro in ('0482394-5','0487949-5','0188620-3','0482483-6','0482476-3')
group by 
t8.sucursal, 
t1.factura, 
t8.cuenta_estilo_ahorro, 
