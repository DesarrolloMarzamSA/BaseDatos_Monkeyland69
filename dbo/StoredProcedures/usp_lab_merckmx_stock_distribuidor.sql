...
GO
CREATE --	CREATE	--	DROP
PROCEDURE [dbo].[usp_lab_merckmx_stock_distribuidor] (@fecha VARCHAR(10))
...
/*
EXECUTE	usp_lab_merckmx_stock_distribuidor '2011-07-10'
SELECT * FROM lab_merckmx_stock_distribuidor
SELECT * FROM lab_merckmx_control_stock_distribucion
*/
...
(SELECT lab_corto FROM lab_fusiones WHERE id_lab = 'MERCKMX')

ALTER TABLE #lab_msd_productos ADD PRIMARY KEY (codigo)

IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_merckmx_stock_distribuidor') 
	TRUNCATE TABLE lab_merckmx_stock_distribuidor
ELSE
		CREATE TABLE lab_merckmx_stock_distribuidor (
...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_TYPE='BASE TABLE' AND TABLE_NAME='lab_merckmx_control_stock_distribucion') 
	TRUNCATE TABLE lab_merckmx_control_stock_distribucion

ELSE
	CREATE TABLE lab_merckmx_control_stock_distribucion (
		TotalRegistros			int						,
		SumaEAN							bigint				,
...
--	and ib.piezas > 0
	...
	INSERT INTO lab_merckmx_stock_distribuidor
...
SET @total_registros	= (SELECT COUNT(*) FROM lab_merckmx_stock_distribuidor)
SET @sum_ean					= (SELECT ISNULL(Sum(convert(bigint,CodigoEAN)),0) FROM lab_merckmx_stock_distribuidor)-- GROUP BY ean
SET @sum_cantidad			= (SELECT ISNULL(Sum(Cantidad),0) FROM lab_merckmx_stock_distribuidor)-- GROUP BY Cantidad

INSERT INTO lab_merckmx_control_stock_distribucion
SELECT
	 @total_registros	TotalRegistros,
	 @sum_ean					SumaEAN,
	 @sum_cantidad		SumaCantidad

UPDATE lab_merckmx_stock_distribuidor SET NumeroLote = '', Texto1 = '', Texto2 = '', Texto3 = ''	,MotivoBloqueo = ''
