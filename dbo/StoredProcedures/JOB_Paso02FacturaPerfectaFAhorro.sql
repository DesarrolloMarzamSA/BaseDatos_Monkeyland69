...
-- Description:	
/*
 EXEC [dbo].[JOB_Paso02FacturaPerfectaFAhorro]
*/
-- =============================================
CREATE PROCEDURE [dbo].[JOB_Paso02FacturaPerfectaFAhorro]
AS
BEGIN
	...
	@TotalMarzam decimal(20,2)=0.00
	 ...
	 WHILE EXISTS(SELECT TOP 1 1 FROM [dbo].[ControlFacturaPerfectaFAhorro] WHERE IdProceso = 2 AND IdEstado in (1,4) AND Procesar = 1)
	 BEGIN 
		SET @IdUsuario = NULL
		SET @Periodo  = NULL
        SET @Id = 0
		...
		SELECT TOP 1 @Id = Id, @IdUsuario = [Usuario], @Periodo = [Periodo] FROM [dbo].[ControlFacturaPerfectaFAhorro] WHERE IdProceso = 2 AND IdEstado in (1,4) AND Procesar = 1
		/*INICIA PROCESO */		
		BEGIN TRY
		    UPDATE [dbo].[ControlFacturaPerfectaFAhorro] SET [Procesar] = 0 WHERE  Id = @Id
				...
				C30_descGravado decimal(20,2), C31_netoGravado decimal(20,2) --,C66_IMPORTE_TASA0 decimal(20,2), C69_Base decimal(20,2)
				)
