CREATE procedure [dbo].[usp_fahorro_remisiones]
as
set nocount on
...
/*
select * into ##encabezado from encabezado where segto = 'C1' and ctepadre = '007' and fechaprog > dateadd(dd, -2, current_timestamp)
select t2.* into ##detalle from encabezado t1 inner join detalle t2 on t1.sucursal = t2.sucursal and t1.factura = t2.factura where t1.fechaprog > dateadd(dd, -2, current_timestamp) and t1.segto = 'C1' and t1.ctepadre = '007'
...
*/
--solucion para cambiar la sucursal 1 a la 21 -Marco Andrade
...
select * into ##encabezado from historica.dbo.encabezado with(nolock) where segto = 'C1' and ctepadre = '007' and fecha_tandem >= convert(smalldatetime, convert(varchar(10), current_timestamp - 2, 121), 121) AND ISNUMERIC(LTRIM(RTRIM(ORDEN))) = 1 --fechaprog >= dateadd(dd, -2, current_timestamp)
--select * into ##encabezado from historica.dbo.encabezado with(nolock) where segto = 'C1' and ctepadre = '007' and fecha_tandem >= convert(smalldatetime, convert(varchar(10), current_timestamp    , 121), 121) AND ISNUMERIC(LTRIM(RTRIM(ORDEN))) = 1 --and factura='30392053' --and sucursal = 3--fechaprog >= dateadd(dd, -2, current_timestamp)


select t2.* into ##detalle from historica.dbo.encabezado t1 with(nolock) inner join historica.dbo.detalle t2 with(nolock) on t1.sucursal = t2.sucursal and t1.serie = t2.serie and t1.factura = t2.factura where t1.fecha_tandem >= convert(smalldatetime, convert(varchar(10), current_timestamp - 2, 121), 121) and t1.segto = 'C1' and t1.ctepadre = '007' AND ISNUMERIC(LTRIM(RTRIM(T1.ORDEN))) = 1 --and t1.sucursal = 3 --t1.fechaprog >= dateadd(dd, -2, current_timestamp) and t1.segto = 'C1' and t1.ctepadre = '007'
create index idx_temp_encabezado on ##encabezado(sucursal, factura, cliente, orden, segto, ctepadre, fechaprog)
create index idx_temp_detalle on ##detalle(sucursal, factura)
create index idx_temp_detalle_dest_det on ##detalle(dest_det)
create index idx_temp_detalle_codigos on ##detalle(codigos)
...
truncate table remisiones_spt_fahorro

insert into remisiones_spt_fahorro
select 
...
--select * from remisiones_spt_fahorro where archivo like '%1077_0193900-5%'
