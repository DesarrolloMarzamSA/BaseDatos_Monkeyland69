...
-- Modificado:  Francisco Roberto Martínez Hernández (28/03/2019)
-- Create date: 01-12-2017
-- Description:	obtiene detalle de Facturacion de Farmacias del ahorro
-- exec [dbo].[usp_ObtenerDetalleRemisionesFAhorroIBS] '2019',''
-- =============================================
CREATE PROCEDURE [dbo].[usp_ObtenerDetalleRemisionesFAhorroIBS] --'2022-12-26','2023-01-05'
@FechaInicial DATE,
@FechaFinal DATE
AS
BEGIN
	...
	SET NOCOUNT ON;
	--DECLARE @FechaInicial  DATE ='2023-01-02',@FechaFinal DATE='2023-01-12'
	DECLARE @FechaI VARCHAR(10),@FechaF VARCHAR(10)
	SET @FechaI=  CONVERT(VARCHAR(10),@FechaInicial,112)
	SET @FechaF=  CONVERT(VARCHAR(10),@FechaFinal,112)
...
inner join MA4620EF04.SRBSOH on IDORNO= OHORNO AND OHCUNO=IHCUNO AND OHORDT IN (''''CS'''',''''FR'''')
...
LEFT JOIN MA4620EF04.SRBGDT AS SROGDT ON SROGDT.DTGDSQ = IDGDSQ AND SROGDT.DTDITY = ''''1'''' 
 ...
 where IHTYPP=1 and  NANCA1=''''99007''''  AND IHIDAT between <FechaI> and <FechaF> 
...
SET @sqlQuery=REPLACE(@sqlQuery,'<FechaI>',@FechaI)
SET @sqlQuery=REPLACE(@sqlQuery,'<FechaF>',@FechaF)
...
update #detalle_fahorroFacturas set HashCode=CHECKSUM([IDINVN],[IDCUNO],[IDLINE],rtrim([IDPRDC]),cast(ihoref as  money) )  WHERE ISNUMERIC(ihoref) = 1
     MERGE detalle_fahorroFacturas P2
	--solo si es necesario por error de usuario agregar  'WHERE ISNUMERIC(ihoref) = 1' a la siguiente consulta
	USING (SELECT * FROM #detalle_fahorroFacturas ) P1
	ON (P1.[IDINVN] = P2.[IDINVN] and P1.[IDCUNO]=P2.[IDCUNO] and P1.[IDLINE]=P2.[IDLINE] and rtrim(P1.[IDPRDC])=rtrim(P2.[IDPRDC]) 
	and cast(P1.ihoref as money)=cast(P2.ihoref as money))
