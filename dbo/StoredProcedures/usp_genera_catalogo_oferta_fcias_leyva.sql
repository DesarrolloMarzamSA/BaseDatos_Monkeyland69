CREATE PROCEDURE [dbo].[usp_genera_catalogo_oferta_fcias_leyva]
	@sucursal TINYINT,
...
SELECT	'',  REPLICATE(' ', 12-LEN(CONVERT(VARCHAR(2), '01'))) + CONVERT(VARCHAR(2), '01') as zona,
		REPLICATE(' ', 13-LEN(CONVERT(varchar(15), CONVERT(BIGINT, t1.cod_barras)))) + CONVERT(varchar(15), CONVERT(BIGINT, t1.cod_barras)) as codigo,
		REPLICATE(' ', 29-len(SUBSTRING(t1.descripcion,0,29)))+ltrim(rtrim(SUBSTRING(t1.descripcion,0,29))) as descripcion, 
		REPLICATE(' ', 8-len(CONVERT(varchar,t1.prec_farm)))+CONVERT(varchar,t1.prec_farm)	  +
		REPLICATE(' ', 6-len(CONVERT(varchar,monkeyland.dbo.udf_calc_descto_prod(clas_fis,19.00,descto_prod))))+CONVERT(varchar,monkeyland.dbo.udf_calc_descto_prod(clas_fis,19.00,descto_prod))as porceFinanciero,
		REPLICATE(' ', 5-LEN(CONVERT(VARCHAR(7), '0'))) + CONVERT(VARCHAR(7), '0') as cantpCargo,
 		REPLICATE(' ', 6-LEN(CONVERT(VARCHAR(7), '0'))) + CONVERT(VARCHAR(7), '0') as cantsCargo,
		REPLICATE(' ', 5-LEN(CONVERT(VARCHAR(7), '0'))) + CONVERT(VARCHAR(7), '0') as limpsCargo,
		REPLICATE(' ', 5-LEN(CONVERT(varchar(15), replace(t1.descto_prod,'.00','')))) + CONVERT(varchar(15), replace(t1.descto_prod,'.00','')) as desProd,
	 	REPLICATE(' ', 8-len(CONVERT(varchar,t1.prec_pub))) +  CONVERT(varchar,t1.prec_pub)   as precPublic,
		REPLICATE(' ', 7-len(CONVERT(varchar,case when t1.iva=0.00 then 0 when t1.iva=0.11 then 11 when t1.iva=0.16 then 16 end)))+CONVERT(varchar,case when t1.iva=0.00 then 0 when t1.iva=0.11 then 11 when t1.iva=0.16 then 16 end) as iva,
