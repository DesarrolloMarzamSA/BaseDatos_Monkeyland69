...
--exec [dbo].[usp_genera_fact_elect_det_farmacon_previa_manual] 5,'20210921','C2','999'

CREATE PROCEDURE [dbo].[usp_genera_fact_elect_det_farmacon_previa_manual] 
	@sucursal INT,
	@fecha datetime,
	@segto VARCHAR(2),
	@ctepadre VARCHAR(3)
...
WHERE	t1.fecha_factura >= dateadd(d, -3, CONVERT(DATETIME, CONVERT(VARCHAR(10), @fecha, 121), 121)) AND 
	...
	exec [192.168.90.209].MiddleWare.[IEmbarque].[ExtraccionFacturacionEstandar] '20210825','','0016001263'

	insert into #facturacion_farmacon
	select  distinct 
		LEFT(CONVERT(VARCHAR(16),  isnull('FC21'+cast(cast(VBELN as bigint) as varchar(12)),'')) + REPLICATE(' ', 16), 16)  as serie_factura,
