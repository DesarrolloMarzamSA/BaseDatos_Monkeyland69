CREATE PROCEDURE [dbo].[usp_genera_fact_elect_det_farmacon_previa_manual_BK06072021]
	@sucursal INT=0,
	@fecha varchar(8)='',
	@segto VARCHAR(2)='',
	@ctepadre VARCHAR(3)=''
AS
--exec [usp_genera_fact_elect_det_farmacon_previa_manual]
--EXECUTE [usp_genera_fact_elect_det_farmacon_previa_manual] 0, '2015-05-06', 'C2', '599'
--EXECUTE [usp_genera_fact_elect_det_farmacon_previa_manual] 5, '2015-05-06', 'C2', '599'
...
WHERE	t1.fecha_factura >= dateadd(d, -3, CONVERT(DATETIME, CONVERT(VARCHAR(10), @fecha, 121), 121)) AND 
	...
	--******************************************************* SAP *********************************************************************
		...
		set @fecha_Aux= convert(varchar,@fecha,112)
	...
	exec [192.168.90.209].MiddleWare.[IEmbarque].[ExtraccionFacturacionEstandar] '20210624','0011000006'

	insert into #facturacion_farmacon
	select  distinct 
		LEFT(CONVERT(VARCHAR(16),  isnull('FC21'+VBELN,'')) + REPLICATE(' ', 16), 16)  as serie_factura,
	...
	from #factura_sap WHERE [VWERK]=@sucursal_aux and VBELN='0900053033'
