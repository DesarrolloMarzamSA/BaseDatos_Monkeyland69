CREATE VIEW dbo.facturacion_electronica_neto
AS
...
FROM            Historica.dbo.fesprueba AS t1 WITH (nolock) LEFT OUTER JOIN
                         capa_ibs.dbo.maestro_productos AS t2 WITH (nolock) ON t1.codigo = t2.codigo

GO

EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'facturacion_electronica_neto';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'facturacion_electronica_neto';
