CREATE VIEW dbo.vi_fact_elec_estandar_precioneto
AS
SELECT        RIGHT('00' + CONVERT(varchar(2), t1.sucursal), 2) + t1.cliente + t1.digito_verificador + CASE WHEN t1.folio_fiscal IS NULL THEN t1.serie + LEFT(RIGHT(t1.factura, 7) 
                         ...
                         CASE WHEN t1.porcentaje_utilidad < 0 THEN 0 ELSE t1.porcentaje_utilidad END), 5) + RIGHT('0000000000000' + CONVERT(varchar(13), CONVERT(numeric(18, 2), 
                         d.IDITET)), 13) + RIGHT('000000000' + REPLACE(t1.orden, ' ', ''), 9) + RIGHT('000000' + CONVERT(varchar(6), t1.porcentaje_iva), 6) 
                         + t1.filler + RIGHT('00000' + CONVERT(varchar(5), t1.no_registro), 5) AS segunda_parte, t1.segto, t1.ctepadre, t1.rfc, t1.fecha_factura, t1.cliente, t1.sucursal, t1.factura, 
                         t1.fecha_tandem, t1.clas_fis, CONVERT(numeric(18, 2), d.IDITET) AS iditit, t1.importe_neto
FROM            dbo.facturacion_electronica_estandar AS t1 INNER JOIN
                         capa_ibs.dbo.detalle_surtidoprueba AS d ON t1.sucursal = d.SUCURSAL AND t1.factura = d.FACTURA AND RIGHT('000000000' + t1.codigo, 9) 
                         = d.CODIGOS INNER JOIN
                         dbo.sucursales AS t2 ON t1.sucursal = t2.sucursal INNER JOIN
                         dbo.maestro_productos AS t3 ON t1.codigo = t3.codigo
WHERE        (CONVERT(varchar, t1.fecha_factura, 112) >= '20160929')
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vi_fact_elec_estandar_precioneto';


GO

EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vi_fact_elec_estandar_precioneto';
