CREATE VIEW dbo.vw_facturacion_benavides
AS
SELECT     TOP (100) PERCENT t1.sucursal, t1.cliente, t1.digito_verificador, t1.serie, t1.factura, t1.fecha_factura, t1.codigo, t1.descripcion, 
...
FROM         dbo.fes_benavides_2013 AS t1 WITH (nolock) LEFT OUTER JOIN
                      capa_ibs.dbo.maestro_productos AS t2 WITH (nolock) ON t1.codigo = t2.codigo
ORDER BY t1.sucursal

GO

EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vw_facturacion_benavides';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vw_facturacion_benavides';
